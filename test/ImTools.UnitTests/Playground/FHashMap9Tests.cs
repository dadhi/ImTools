...
[TestFixture]
public class FHashMap9Tests
{
    internal static void Verify<K, V, TEq>(FHashMap9<K, V, TEq> map, IEnumerable<K> expectedKeys)
    ...
    public void Real_world_test_AddOrUpdate_NO_Resize()
    {
        ...
        var map = new ImTools.Experiments.FHashMap9<Type, string, RefEq<Type>>(128);

        foreach (var key in types)
            map.AddOrUpdate(key, "a");

        map.AddOrUpdate(typeof(FHashMap9Tests), "!");
    ...
    {
        var map = new FHashMap9<int, string, IntEq>(16);
